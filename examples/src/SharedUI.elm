...
module SharedUI
    exposing
        ...
        , makeMsgCmd, backendCmd
        , getProperties, setProperties, getDbDict, setDbDict
        )
    ...
    case msg of
        ...
        Login ->
            case model.profile of
                Nothing ->
                    ( model, DB.login (mdb model) model )
        ...
        Logout ->
            case model.profile of
                ...
                Just _ ->
                    ( model, DB.logout (mdb model) model )
        PartialLogout ->
            case model.profile of
            ...
            case model.profile of
                ...
                Just profile ->
                    case model.key of
                        "" -> ( model, Cmd.none )
                        key ->
                            ( model, DB.get profile.userId key (mdb model) model )
        Put ->
            case model.profile of
                Nothing -> ( model, Cmd.none )
                Just profile ->
...
sharedView subheader model =
    div
        ...
        , div [ id "amazon-root" ] []
          --this id is required by the Amazon JavaScript
